...
- the "withform" option, outsde of input elements, also replicates functionality of the following form attributes : "action", "formaction", "method", "formmethod". There is special functionality for the event's currentTarget, where its' values take priority.
- if a "data-method", "method", or "formmethod" is not active, dive reverts to the "POST" method.
- by default dive uses e.preventDefault(). You can disable this with the "allowDefault" option.
- I am testing alternate "behaviors" currently we have the repeat, stagger and delay behaviors. I think I will remove the rename the delay to stagger and get rid of the current stagger, whos name and functionaliy is confusing. Be warned. See changelog section 3.0.0 for more.
